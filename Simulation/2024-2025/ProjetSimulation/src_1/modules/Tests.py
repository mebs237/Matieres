...
-----
    ...
    - ``Tests`` : classe abstraite servant d'interface autres classes , les obligeant à implémenter la méthode ``run_test`` , fonction principale qui exécute le test statistique.
...
Utilisation
------
    Pour une ``sequence`` , on teste l'hypothèse **H₀ : 'la séquence suit une distribution uniforme '** avec le test statistique T en :

    - *Instanciation* : créer une instance  ``T`` de la classe representant le test voulu , en fixant ou pas les paramètres du test ; si des paramètres ne sont pas spécifiés , les parramètres par défaut seront utilisés
    - *Application* : appliquer la méthode ``run_test`` sur ``sequence`` par ``T.run_test(sequence)`` ; cette méthode retourne un dictionnaire contenant les résultats du test
    - *Interpretation* : la valeur ``accept`` du resultat obtenu sera ``True`` ou ``False`` si on accepte l'hypothèse nulle H₀ (ou plus précisément , on ne rejette pas cette hypothése)
...
-------
    ...
    >>> test = Chi2Test()
    >>> res_test = test.run_test(sequence , alpha = 0.01)
...
from typing import Dict, Optional , Tuple , Callable , Literal
...
class Result:
    ...
    """
    stat_obs: float = field(default=np.nan)
    stat_crit: float = field(default=np.nan)
    p_value: float = field(default=np.nan)
    accept: bool = field(default=np.nan)
    add_info: Optional[Dict] = field(default_factory=lambda:dict)
...
class Tests(ABC):
    ...
    def discretize(data: NDArray ,
                   ...
                   binning : Literal["quantile" , "uniform"] = "quantile") -> Tuple[NDArray, int]:
        """
        ...
        d = n_bins if not n_bins is None else max(6 , int(1 + np.log2(len(data))) )  # règle de Sturges
...
class Chi2Test(Tests):
    ...
    Args:
        nb_bins : Nombre de catégories (défaut: 10)
    ...
    """
    MIN_SAMPLE_SIZE = 20 # nombre minimal d'éléments que doit contenir une séquence pour que le test du χ² soit valide
    MIN_DEGRE_FREEDOM = 1 #dégré de liberté minimum
    ...
    def __str__(self) -> str:
        return f"Chi2Test(nbbins={self.nb_bins})"
    ...
    def get_counts(data: NDArray,
        ...
        """
        effectue les comptages d'effectifs selon chacune des ``nb_bins`` classes
        ...
        observed , _ = np.histogram(data, bins=nb_bins)
        if probabilities is None:
            # Distribution uniforme
            expected = np.full(nb_bins, n / nb_bins)
        else:
            # Utiliser les probabilités fournies
            probabilities = np.array(probabilities)
            if len(probabilities) != nb_bins:
    ...
    def run_test(self,
        ...
        observed, expected = self.get_counts(data, nb_bins=self.nb_bins, probabilities=self.probabilities)
        ...
        if n<self.MIN_SAMPLE_SIZE or df < self.MIN_DEGRE_FREEDOM:
            return Result(add_info={"note":f"séquence trop petite pour un test fiable : n={n} , df = {df}"})
        ...
        if info :
            low_expected = np.sum(expected<self.min_expected)
            result.add_info = {"df" : df ,
                        "observed" : observed,
                        "expected" : expected,
                        "note":f"{low_expected} classes avec effectif <{self.min_expected}"if low_expected else f"les classes ont chacune un effectif>={self.min_expected}"
                       }

        return result

    @staticmethod
    def run_test2(observed: NDArray,
              expected: NDArray,
              alpha: float = 0.05,
              info: bool = False
              ) -> Result:
        ...
        if n<Chi2Test.MIN_SAMPLE_SIZE or df < Chi2Test.MIN_DEGRE_FREEDOM:
            return Result(add_info={"note":f"séquence trop petite pour un test fiable : n={n} , df = {df}"})
        ...
        if info:
            ...
            result.add_info = {"df" : df ,
                        "observed" : observed,
                        "expected" : expected,
                        "note":f"{low_expected} classes avec effectif <5"if low_expected else "les classes ont chacune un effectif>=5"
                       }
...
class GapTest(Tests):
    ...
    def run_test(self,
        ...
        if len(indices) <= 1:
            return Result(add_info={"note1": "Pas assez de marques pour calculer des gaps"})
        ...
        result =  Chi2Test.run_test2(gap_counts,expected_counts, alpha=alpha,info=info)
...
class PokerTest(Tests):
    ...
    def __init__(self, group_size: int = 5,n_bins=None) -> None:
        ...
        if n_bins is not None and n_bins < group_size + 1:
            raise ValueError(f"Le nombre de catégories/taille de l'alphabet doit être au moins {group_size + 1} pour une bonne validitédu test")
        self.group_size = group_size
        self.n_bins = n_bins if n_bins is not None else group_size + 1

    def __str__(self) -> str:
        return f"PokerTest(groupsize={self.group_size},nbbins={self.n_bins})"
    ...
    def run_test(self,
        ...
        result = Chi2Test.run_test2(observed, n_group * probs, alpha=alpha, info=info)
...
class CouponCollectorTest(Tests):
    ...
    max_seg : int
        longueur maximale des segments à considérer
    """
    def __init__(self, nb_coupons: Optional[int] = None,max_length:int = 30):
        """
        Initialise le test du Collectionneur de Coupons.

        """
        self.nb_coupons = nb_coupons
        self.max_length = max_length

    def __str__(self) -> str:
        return f"CouponCollectorTest(nbcoupon={self.nb_coupons})"
    ...
    def run_test(self,
        ...
        # Discrétisation
        data_new , nb_coupons  = Tests.discretize(data=data , n_bins = self.nb_coupons)

        t = max(self.max_length,nb_coupons)
        ...
        for value in data_new:
            ...
            lenght += 1
            # si le segment est trop long ou s'il est complet ,  on le coupe et on le stocke
            if lenght >=t or (len(current_segment)==nb_coupons):
        ...
        for i in range(size-1):
            r = nb_coupons+i # longueur de segment
            probs[i] = (factorial(nb_coupons) / (nb_coupons ** r)) * Tests.stirling(r - 1, nb_coupons - 1)
        ...
        result = Chi2Test.run_test2(observed=observed_counts,expected=expected, alpha=alpha, info=info)

        if info :
            result.add_info.update( {
                "length_segments":len_segments ,
                "nb_coupons": nb_coupons,
                "max_length":t})
...
class KSTest(Tests):
    ...
    def __str__(self) -> str:
        return "KSTest(Uniform)"if self.cdf is None else "KSTest(customed)"
...
class MaximumTest(Tests):
    ...
    def __init__(self, group_size: int = 5):
        ...
        """
        if group_size < 2:
            raise ValueError("group_size doit être au moins 2")
        self.group_size = group_size

    def __str__(self) -> str:
        return f"MaximumTest(groupsize={self.group_size})"
    ...
    def run_test(self,
        ...
        if n < self.group_size:
            raise ValueError(f"La taille de l'échantillon ({n}) doit être au moins égale à group_size ({self.group_size})")

        # Diviser les données en groupes de taille group_size

        groups = [data[i:i + self.group_size] for i in range(0 , n - self.group_size + 1 , self.group_size)]
        ...
        result = kstest.run_test(data = max_values ,
                               alpha=alpha ,
                               info=info)
        if info :
            result.add_info.update({'max_df':self.group_size-1,
