...
from .Tests import Tests , Chi2Test , GapTest , MaximumTest , KSTest
...
class level(Enum):
    """niveaux d'abstraction possibles pour l'analyse statistique d'un générateur"""
    Global = "Global"
    Bytest = "Test"
    Bysize = "Size"
    Byiter = "Iteration"
...
def analyse_sequence(sequence: NDArray,
                     ...
                     log:bool=False) -> ComputeRes:
    """
    fait les calculs de l'analyse de la séquence
    """

    tests = tests if tests else [Chi2Test(), MaximumTest() , GapTest()]
    ...
    for i, test in enumerate(tests):
        if log :
            percent = (i+1)/len(tests)
            print(f"compute for {test}...{percent:.1%}...")
...
def evaluate_generator(generator : Generators ,
    ...
    """
    tests = tests if tests else [Chi2Test(), KSTest(), MaximumTest() , GapTest()]
    all_results = []
    if log:
        percent = 0.0
        print(f"{percent:.1%}")
