...
class OurGenerator(Generators):
    ...
    _iner_counter_ = 0  # compteur d'instance

    def __str__(self)->str:
        return f"OurGenerator_{self.suffix}"

    def __init__(self, decimals: NDArray,
                 a: int = 1664525,
                 c: int = 1013904223,
                 m: int = 2**32 -1 ,
                 suffix:str=None):
        ...
        self.m = m
        if suffix is None:
            self.suffix = str(self.__class__._iner_counter_)
            self.__class__._iner_counter_+=1
        else:
            self.suffix = suffix
