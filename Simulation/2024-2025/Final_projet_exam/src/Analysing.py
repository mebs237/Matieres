...
DEFAULT_METRICS = [
    ...
    ScoreMetric(
        name="mean_p_value",
        func=lambda r: r.stats.get["mean_p_value"],
...
class AnalysisResult:
    ...
    def generate_report(self) -> str:
        """
        Génère un rapport d'interprétation  basé des résultats obtenus
        """

        report = "#  Rapport d'interprétation de "+ self.name+ "\n\n"
        ...
        for test_name, stats in self.stats.items():
            ...
            report += self._interpret_stats(self.stats)
...
def analyse_sequence(sequence:NDArray ,
                     ...
                     alpha : float = 0.05 ,
                     granularities:List[Union[int, float]] = 0.1,
                     name : str = None
                     )->AnalysisResult:
    """
    ...
    hist_df = DataFrame(results,index=False)
    accept_ratio = hist_df["accept"].mean()
    rejection_ratio = 1-accept_ratio
    stats = {
...
def to_wind_size(granularities:List[Union[int,float]],
    ...
    """
    Transforme une liste de granularités donnée en taille entière de sous séquence
    """
    ...
    if np.all([isinstance(g,int)and 0<g<=len_seq for g in granularities]) :
        ...
        raise ValueError(f"les granularités >0 doiventt être soit des entiers <={len_seq} , soit des fractions  <=1 de la taille de ")
