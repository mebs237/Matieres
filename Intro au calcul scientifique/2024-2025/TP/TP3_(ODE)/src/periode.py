...
def periode(epsilon:float ,mu:float = 0, t_max:float = 50, y0 : Optional[NDArray] = None )->Dict[str,Union[float,NDArray]]:
    """
    ...
    Strategie
    ---------
        - on repère les temps tᵢ  entre deux extrema de x (qui correspondent aux racines de ∂ₜx(t) )
        - on calcul les différences entre Tᵢ = tᵢ₊₁ - tᵢ
        - on prend  T = la moyenne des Tᵢ . Pour cela :
    """
    if y0 is None:
    ...
    if len(t_events)<2:
        period = None # pas assez d'extrema détectés
    else :
    ...
    return {'period':period , 'sol':sol , 'y0':y0}



def vitesse_critique(epsilon:float, mu:float=0.0 , x_target:float = 5.0 , atol:float=1e-12 , v0_max:float=50 , n_iter : int = 100)->Dict:
    """
    ...
    - pour **mu** > 0 : recherche par dicothomie
    ...
    info : bool, optional
        si true retourne la vitesse initiale maximale et les informations supplémentaires

    Returns
    -------
    float , Optional[Dict]
        vitesse initiale maximale pour que la position x ne dépasse pas ``x_target``
        si info = True, retourne aussi la cause d'arrêt de la recherche dichotomique et le nombre d'itérations
    ...
    if mu == 0:
        ...
        # On cherche la vitesse initiale v0 telle que x_max_sim = x_max
        while (not condv) and i<n_iter:
            ...
            if condx :
                return {'v_crit': v_mid ,
                            'add_info':{'x_target atteint':condx ,
                                        'iter exce': i>= n_iter ,
                                        'iter': i
                                        }
                            }
        ...
        return {'v_crit': v_mid ,
                'add_info':{'précision atteint':condv ,
                            'iter exce': i>= n_iter ,
                            'iter': i
                            }
                }
