...
def Jac(u:vector,E:float)->vector:
    ...
    if g == 0 and E == 0:
        ...
        df1_dg = Rg*(1-g-0.6*w*(E+g)/(0.31*E+g)+g*(-1-0.6*w*(-0.69*E)/(0.31*E+g)**2))
        df2_dg = Rw* w*(-1.07*(0.31*E+g)/(E+g)-1.07*g*(0.69*E)/(E+g)**2)
        df1_dw = Rg*g*(-0.6*(E+g)/(0.31*E+g))
        df2_dw = Rw*(1-w-1.07*g*(0.31*E+g)/(E+g)-w)
...
def ulim(u0:vector, f:Callable=equation, t_max:int=100, E:float=0.00):
    """
    ...
    u0 = array(u0)
    try :
        sol = solve_ivp(f, [0, t_max], u0, method='RK45', rtol=1e-8, atol=1e-8 ,args = (E))
...
def plot_separatrix(E:float):
    ...
    for eq  in classes.keys():
        if classes[eq] == 'stable':
            plt.plot(eq[0], eq[1], 'go', markersize=16, label='Équilibre stable')
        elif classes[eq] == 'instable':
            plt.plot(eq[0], eq[1], 'ro', markersize=16, label='Équilibre instable')
        else:  # selle
            plt.plot(eq[0], eq[1], 'bo', markersize=16, label='Point selle')
